...
export class DORateLimitStore extends DurableObject<Env> {
  ...
  async increment(
    ...
    const burstBuckets = config.burst
      ...
      : [];
    const dailyBuckets = config.dailyLimit
      ? this.getBucketsInWindow(key, now, dailyWindow, bucketSize)
      : [];
    ...
    const dailyCount = dailyBuckets.reduce(
      (sum, bucket) => sum + bucket.count,
      0,
    );
  ...
  async getRemainingLimit(
    ...
    if (config.dailyLimit) {
      const dailyBuckets = this.getBucketsInWindow(
        key,
        now,
        dailyWindow,
        bucketSize,
