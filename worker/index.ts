...
import { CodeGeneratorAgent as BaseCodeGeneratorAgent } from './agents/core/codingAgent';
import { ensurePreviewDomain } from './middleware/ensure-preview-domain';
import { ensureNonIpHostname } from './middleware/ensure-non-ip-hostname';
import { showRoutes } from 'hono/dev';


// Durable Object and Service exports
export { UserAppSandboxService } from './services/sandbox/sandboxSdkClient';
import { UserSecretsStore as BaseUserSecretsStore } from './services/secrets/UserSecretsStore';

export const UserSecretsStore = Sentry.instrumentDurableObjectWithSentry(
  sentryOptions,
  BaseUserSecretsStore,
);
export const CodeGeneratorAgent = Sentry.instrumentDurableObjectWithSentry(
  sentryOptions,
  BaseCodeGeneratorAgent,
);
export const DORateLimitStore = Sentry.instrumentDurableObjectWithSentry(
  sentryOptions,
  BaseDORateLimitStore,
);
...
/**
 * Handles requests for the main domain (platform host).
 */
async function handleMainDomainRequest(
  ...
  if (pathname.startsWith('/api/proxy/openai')) {
    ...
    const origin = request.headers.get('Origin');
    const previewDomain = getPreviewDomain(env, hostname);
 ...
 */
const routingApp = new Hono<{ Bindings: Env }>();

// 1. Critical configuration check: Ensure custom domain is set.
routingApp.use(ensurePreviewDomain);

// 2. Security: Immediately reject any requests made via an IP address.
routingApp.use(ensureNonIpHostname);


routingApp.use(async (c, next) => {
  const url = new URL(c.req.url);
  const { hostname } = url;
  const previewDomain = getPreviewDomain(c.env, hostname);
  const isSubdomainRequest = hostname.endsWith(`.${previewDomain}`) || (hostname.endsWith('.localhost') && hostname !== 'localhost');
  const isMainDomainRequest = hostname === c.env.CUSTOM_DOMAIN || hostname === 'localhost';
  if (isMainDomainRequest) {
    return handleMainDomainRequest(c.req.raw, c.env, c.executionCtx);
  }
  if (isSubdomainRequest) {
    return handleUserAppRequest(c.req.raw, c.env);
  }
  return next();
});

routingApp.notFound(async (c): Promise<Response> => {
  return c.json({ error: 'Not Found' }, 404);
});


showRoutes(routingApp);
// Wrap the entire worker with Sentry for comprehensive error monitoring.
export default Sentry.withSentry(sentryOptions, routingApp);
