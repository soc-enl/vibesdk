...
export class VibeClient {
  ...
  async build(
    ...
    const session = new BuildSession(start, {
      ...
      ...(options.credentials
        ? { defaultCredentials: options.credentials }
        : {}),
    });
  ...
  async connect(
    ...
    const data = await this.http.fetchJson<
      ApiResponse<{ websocketUrl: string; agentId: string }>
  ...
  apps = {
    ...
    listPublic: async (query: PublicAppsQuery = {}) => {
      ...
      return this.http.fetchJson<
        ApiResponse<{ apps: AppListItem[]; pagination?: unknown }>
      >(`/api/apps/public${qs}`, {
        method: 'GET',
        headers: await this.http.headers(),
      });
    ...
    listMine: async () => {
      return this.http.fetchJson<
        ApiResponse<{ apps: AppWithFavoriteStatus[] }>
      >('/api/apps', {
        method: 'GET',
        headers: await this.http.headers(),
      });
    ...
    get: async (appId: string) => {
      return this.http.fetchJson<ApiResponse<AppDetails>>(
        `/api/apps/${appId}`,
        {
          method: 'GET',
          headers: await this.http.headers(),
    ...
    delete: async (appId: string) => {
      return this.http.fetchJson<ApiResponse<DeleteResult>>(
        `/api/apps/${appId}`,
        {
          method: 'DELETE',
          headers: await this.http.headers(),
    ...
    setVisibility: async (appId: string, visibility: AppVisibility) => {
      return this.http.fetchJson<ApiResponse<VisibilityUpdateResult>>(
    ...
    toggleStar: async (appId: string) => {
      return this.http.fetchJson<ApiResponse<ToggleResult>>(
        `/api/apps/${appId}/star`,
        {
          method: 'POST',
          headers: await this.http.headers(),
