...
export class HttpClient {
  private cachedAccessToken: { token: string; expiresAtMs: number } | null =
    null;
  ...
  private async ensureAccessToken(): Promise<string | undefined> {
    ...
    if (
      ...
      this.cachedAccessToken.expiresAtMs - skewMs > now
    ) {
  ...
  async fetchJson<T>(path: string, init?: RequestInit): Promise<T> {
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          const error = new Error(
            `HTTP ${resp.status} for ${path}: ${text || resp.statusText}`,
          );
    ...
    throw (
      lastError ?? new Error(`Failed after ${this.retryCfg.maxRetries} retries`)
    );
  }
  ...
  async getWsTicket(
    agentId: string,
  ): Promise<{ ticket: string; expiresIn: number; expiresAt: string }> {
    const resp = await this.fetchJson<
      ApiResponse<{ ticket: string; expiresIn: number; expiresAt: string }>
    >('/api/ws-ticket', {
      method: 'POST',
