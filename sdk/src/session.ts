...
export type BuildSessionConnectOptions = Omit<
  AgentConnectionOptions,
  'credentials'
> & {
...
export class BuildSession {
  ...
  readonly wait = {
    generationStarted: (options: WaitOptions = {}) =>
      this.waitForGenerationStarted(options),
    generationComplete: (options: WaitOptions = {}) =>
      this.waitForGenerationComplete(options),
    phase: (options: WaitForPhaseOptions) => this.waitForPhase(options),
    deployable: (options: WaitOptions = {}) => this.waitForDeployable(options),
    previewDeployed: (options: WaitOptions = {}) =>
      this.waitForPreviewDeployed(options),
    cloudflareDeployed: (options: WaitOptions = {}) =>
      this.waitForCloudflareDeployed(options),
  };
   ...
   */
  async connect(
    options: BuildSessionConnectOptions = {},
  ): Promise<AgentConnection> {
    if (this.connection) return this.connection;

    const { autoRequestConversationState, credentials, ...connectionOptions } =
      options;
  ...
  ): Promise<WsMessageOf<PhaseEventType>> {
    return await this.waitForMessageType(
      options.type,
      options.timeoutMs ?? this.getDefaultTimeoutMs(),
    );
  ...
  async waitForDeployable(
    options: WaitOptions = {},
  ): Promise<SessionDeployable> {
  ...
  async waitForPreviewDeployed(
    ...
    const msg = await this.waitForWsMessage(
      (m) =>
        m.type === 'deployment_completed' || m.type === 'deployment_failed',
      timeoutMs,
    );
  ...
  async waitForCloudflareDeployed(
    ...
    const msg = await this.waitForWsMessage(
      (m) =>
        m.type === 'cloudflare_deployment_completed' ||
        m.type === 'cloudflare_deployment_error',
      timeoutMs,
    );
    if (msg.type === 'cloudflare_deployment_error') {
      throw new Error(
        (msg as WsMessageOf<'cloudflare_deployment_error'>).error,
      );
    }
  ...
  private assertConnected(): void {
    if (!this.connection) {
      throw new Error(
        'BuildSession is not connected. Call session.connect() first.',
      );
    }
  }
}
