...
export async function testFullBuild(
  config: TestConfig,
  ...
  try {
    ...
    const { result: buildSession, stepResult: buildStep } = await runStep(
      ...
      async () => {
        return client.build(
          'Build a simple hello world page with a title and welcome message',
          {
            projectType: 'app',
            autoGenerate: true,
          },
    ...
    session.on('ws:close', (e: AgentEventMap['ws:close']) =>
      log?.(`WS: close code=${e.code} reason=${e.reason}`),
    );
    session.on('ws:error', (e: AgentEventMap['ws:error']) =>
      log?.(`WS: error ${e.error}`),
    );
    session.on('ws:reconnecting', (e: AgentEventMap['ws:reconnecting']) => {
      log?.(
        `WS: reconnecting attempt=${e.attempt} delay=${e.delayMs}ms reason=${e.reason}`,
      );
    });
    ...
    const { stepResult: implStartStep } = await runStep(
      'phase_implementing',
      async () => {
        const msg = await session!.wait.phase({
    ...
    const { stepResult: filesStep } = await runStep(
      ...
      async () => {
        ...
        return {
          fileCount: session!.files.listPaths().length,
          files: session!.files.listPaths().slice(0, 5),
        };
      },
      (r) => r,
      log,
    ...
    const previewUrl =
      finalState.previewUrl || (deployStep.details?.previewURL as string);
