...
const apiKeyAvailable = !!(
  process.env.VIBESDK_API_KEY || process.env.VIBESDK_INTEGRATION_API_KEY
);
const runIntegration =
  process.env.VIBESDK_RUN_INTEGRATION_TESTS === '1' || apiKeyAvailable;
...
function logStepResults(result: FullTestResult, prefix: string): void {
  for (const step of result.steps) {
    ...
    console.log(
      `${prefix} [${status}] ${step.step} (${step.duration}ms)${details}${error}`,
    );
  }
}
