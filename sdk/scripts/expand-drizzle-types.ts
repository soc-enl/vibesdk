...
function parseTableDefinition(
  ...
  const columnRegex =
    /(\w+):\s*import\("drizzle-orm\/sqlite-core"\)\.SQLiteColumn<\{([^>]+)\}>/gs;
  let match;
...
function main() {
  ...
  console.log(
    `Found ${inferSelectRefs.size} tables with $inferSelect references`,
  );

  // Process each table
  for (const [tableName, typeNames] of inferSelectRefs) {
    console.log(
      `  Processing table: ${tableName} (types: ${typeNames.join(', ')})`,
    );

    const tableInfo = parseTableDefinition(content, tableName);
    if (!tableInfo) {
      console.warn(
        `    Warning: Could not parse table definition for ${tableName}`,
      );
      continue;
    }
