...
export function usePresentationFiles(
  allFiles: FileType[] = [],
  ...
  const slideFiles: SlideInfo[] = useMemo(() => {
    ...
    const manifestSlideNames = loadManifestSlides(
      `${slideDirectory}/manifest.json`,
    ...
    const slideDirPrefix = slideDirectory.endsWith('/')
      ? slideDirectory
      : `${slideDirectory}/`;
