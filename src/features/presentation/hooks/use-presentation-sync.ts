...
export function usePresentationSync(
  ...
  const [generatingSlides, setGeneratingSlides] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    const isSlideFile = (path: string) =>
      path.startsWith(`${slideDirectory}/`) && path.endsWith('.json');
    ...
    const changedFiles = allFiles.filter((file) => {
      if (!(isSlideFile(file.filePath) || isGlobalFile(file.filePath)))
        return false;

      // Skip files currently being generated/streamed
      // These are handled by the streaming event forwarding
      if (file.isGenerating || generatingSlides.has(file.filePath))
        return false;
