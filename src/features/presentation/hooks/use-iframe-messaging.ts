...
export function useIframeMessaging(
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      if (
        !['file_generating', 'file_chunk', 'file_generated'].includes(
          detail.type,
        )
      )
        return;
      ...
      try {
        ...
        console.error(
          'Failed to forward presentation file event to iframe',
          error,
        );
      }
      ...
      const slide = slideFiles.find(
        (s) =>
          detail.path &&
          (s.filePath === detail.path || detail.path.endsWith(s.filePath)),
      );
