...
export function SecretsManager({ className, compact = false, id }: Props) {
  ...
  const handleViewSecret = async (secret: SecretListItem) => {
    ...
    try {
      ...
      setViewingSecret((prev) =>
        prev ? { ...prev, value: result?.value || 'Unable to decrypt' } : null,
      );
    } catch (err) {
      console.error('Failed to get secret value:', err);
      setViewingSecret((prev) =>
        prev ? { ...prev, value: 'Error loading value' } : null,
      );
    } finally {
      ...
      <>
        <Card id={id} className={className}>
          <CardHeader>
            ...
            <CardDescription>
              Set up your encrypted vault to securely store API keys and
              secrets.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <>
        <Card id={id} className={className}>
          <CardHeader>
            ...
            <CardDescription>
              Unlock your vault to view and manage secrets.
            </CardDescription>
          </CardHeader>
        ...
        <AlertDialog open={resetConfirmOpen} onOpenChange={setResetConfirmOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This deletes all vault secrets and configuration. This cannot be
                undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <CardContent>
        {isLoading && secrets.length === 0 ? (
          ...
          <div className="space-y-2">
            {secrets.map((secret) => {
              const providerKey = secret.metadata?.provider as
                | string
                | undefined;
